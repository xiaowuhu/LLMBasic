...
if __name__=="__main__":
    ...
    num_steps = [500, 50, 20]
    model_names = ["IMDb_RNN_1_500.pth", "IMDb_RNN_1_50.pth", "IMDb_RNN_1_20.pth"]
    for i in range(3):
        num_epochs, batch_size, num_hiddens = 20, 64, 128
        embed_size = 100
        train_data, test_data, vocab = load_data_imdb(batch_size, num_steps[i])
        DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        net = RNN(len(vocab), embed_size, num_hiddens)
