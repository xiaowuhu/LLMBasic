...
def sample_sequence(model, context, max_length, n_ctx, tokenizer: Tokenizer, temperature=1.0, top_k=30, top_p=0.0, repitition_penalty=1.0,
    ...
    with torch.no_grad():
        for i in trange(max_length):
            ...
            result = filtered_logits[filtered_logits > 0]
            while True:
...
def main():
    ...
    parser.add_argument('--length', default=4, type=int, required=False, help='最大生成长度')
    parser.add_argument('--nsamples', default=10, type=int, required=False, help='生成几个样本')
    parser.add_argument('--temperature', default=2, type=float, required=False, help='生成温度')
    ...
    parser.add_argument('--prefix', default='[BOS]春风吹绿文明树[SEP]秋月映红诚信花[SEP]五讲四美 \n [BOS]春风一过江南绿[SEP]秋月初升海上明[SEP]', type=str, required=False, help='对联的上联')
    ...
    file = "../model/ch6/couplet/bpe-couplet-10000.json"
