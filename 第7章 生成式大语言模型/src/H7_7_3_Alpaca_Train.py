...
class InstDataset(torch.utils.data.Dataset):
    def __init__(self, tokenizer: BertTokenizer, ds):
        self.tokenized = []
        for data in tqdm(ds):
            new_data = "[INST]" + data['instruction'] + "[/INST]" + "\\n" + "[INPUT]" + data["input"] + "[/INPUT]" + "\\n" + data['output']
...
def load_data(path, split):
    path = os.path.join(path, f"{split}.json")
...
if __name__=="__main__":
    ...
    tokenizer.add_special_tokens({"additional_special_tokens":["[INST]","[/INST]","[INPUT]","[/INPUT]"]})
    tokenizer.save_pretrained("../model/ch7/alpaca/new_vocab")
    ...
    ds_train = load_data("../data/alpaca-chinese", "train")
    train_dataset = InstDataset(tokenizer, ds_train)
    ...
    ds_valid = load_data("../data/alpaca-chinese", "valid")
    valid_dataset = InstDataset(tokenizer, ds_valid)
